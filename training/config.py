...
def get_colab_a100_config() -> FullConfig:
    """Конфигурация для Colab A100 (40GB VRAM) с packing"""
    config = FullConfig()
    ...
    config.training.gradient_checkpointing = "unsloth"  # "unsloth" быстрый, True = медленный!
    config.lora.r = 64
    config.lora.lora_alpha = 128
    return config


def get_colab_h100_config() -> FullConfig:
    """Configuration for Colab H100 (80GB VRAM): balanced speed and memory."""
    config = FullConfig()
    config.model.base_model = "Qwen/Qwen3-4B"
    config.model.load_in_4bit = True
    config.model.max_seq_length = 1024
    config.training.num_train_epochs = 1
    # Memory-safe defaults for Qwen3-4B + packing in Colab.
    ...
    config.training.gradient_checkpointing = "unsloth"  # ВАЖНО: True = медленный PyTorch, "unsloth" = быстрый
    config.lora.r = 64
    config.lora.lora_alpha = 128
    config.model_name = "tajik-poetry-4b"
    return config
...
def get_config(preset: str = "colab_t4") -> FullConfig:
    ...
    presets = {
        ...
        "colab_h100": get_colab_h100_config,
